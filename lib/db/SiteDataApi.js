...
export async function fetchGlobalAllData({
  ...
  // Coalesce every field against EmptyData's defaults so a missing or explicitly
  // undefined value falls back to a safe empty one instead of undefined —
  // Next.js static export rejects undefined in getStaticProps output. Object
  // spread alone isn't enough here: it only fills in *absent* keys, but a
  // partially-failed fetch (or a stale cache entry written before a field
  // existed) can leave a key *present* with value undefined, which spread
  // would still let win. This bit `notice` and `categoryOptions` in separate
  // deploys already; coalescing every field once covers all of them instead of
  // chasing each one individually as it surfaces.
  const defaults = EmptyData(pageId)
  const merged = { ...defaults, ...cachedData }
